<script setup>
/**
 * SystemBanner.vue — 全站系統公告 / 斷線橫幅（Admin 深色版）
 *
 * 位於整頁最頂端，顯示時把整頁往下推，隱藏時不占版面。
 * 於 App.vue 的 RouterView 之前掛載一次。
 * 對應通知規格 a：系統錯誤（danger）；b：狀態提示（warning / info）。
 */
...
/* 型別配色（Admin 深色主題） */
.sb-warning {
  background-color: var(--tap-warning, #f59e0b);
  color: #1e293b;
}
.sb-danger {
...
.sb-info {
  background-color: rgba(229, 115, 70, 0.18);
